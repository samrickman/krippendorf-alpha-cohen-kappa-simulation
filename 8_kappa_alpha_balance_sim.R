...
prop_negative_class_vec <- seq(from = 0.1, to = 0.9, by = 0.1)


# Simulate data
kappa_alpha_list <- lapply(prop_negative_class_vec, \(prop_negative_class) {
...
kappa_alpha_long <- kappa_alpha_df |>
    ...
    pivot_wider(
        id_cols = c(prop_negative_class, greek_letter),
        names_from = "measure"
    )

nudge_x <- rep(c(0, 0.02), nrow(kappa_alpha_long))
plot_title <- "Comparison between Krippendorf's alpha and Cohen's Kappa with imbalanced classes"
plot_subtitle <- "0.2 error rate, 300 samples"
plot_caption <- "Note: some randomness as confidence estimates created with bootstrapping"


ggplot(kappa_alpha_long, mapping = aes(
    x = prop_negative_class
)) +
    geom_point(
        aes(
            y = estimate,
...
ggsave("./plots/kappa_alpha_comparison/line_plot_prop_negative_class.png", w = 12, h = 7.5)
...
ggsave("./plots/kappa_alpha_comparison/scatter_prop_negative_class.png", w = 12, h = 7.5)

write.csv(kappa_alpha_long, "./simulated_data/8_kappa_alpha_prop_negative_class.csv", row.names = FALSE)